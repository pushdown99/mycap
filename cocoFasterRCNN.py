...
class Model:
  def render_anchors(self):
    ...
    training_data = coco.Dataset(dir = options.dataset_dir, split = options.train_split, augment = False, shuffle = False)
  ...
  def sample_rpn_minibatch(self, rpn_map, object_indices, background_indices, rpn_minibatch_size):
    options = self.options
  ...
  def convert_training_sample_to_model_input(self, sample, mode):
    options = self.options
    ...
    # truth RPN map
    gt_rpn_minibatch_map = self.sample_rpn_minibatch (
      rpn_map = gt_rpn_map,
  ...
  def evaluate(self, model, eval_data = None, num_samples = None, plot = False, print_average_precisions = False):
    options = self.options

    if eval_data is None:
      eval_data = coco.Dataset(dir = options.dataset_dir, split = options.eval_split, augment = False, shuffle = False)
    ...
    for sample in tqdm(iterable = iter(eval_data), total = num_samples):
      x, image_data, _ = self.convert_training_sample_to_model_input(sample = sample, mode = "infer")
    ...
    if print_average_precisions:
      precision_recall_curve.print_average_precisions(class_index_to_name = coco.Dataset.class_index_to_name)
    ...
    if plot:
      precision_recall_curve.plot_average_precisions(class_index_to_name = coco.Dataset.class_index_to_name)
  ...
  def train(self, model):
    ...
    training_data = coco.Dataset(dir = options.dataset_dir, split = options.train_split, augment = not options.no_augment, shuffle = True, cache = options.cache_images)
    eval_data = coco.Dataset(dir = options.dataset_dir, split = options.eval_split, augment = False, shuffle = False, cache = False)
  ...
  def _predict(model, image_data, image, show_image, output_path):
    ...
    visualize.show_detections(
      ...
      image = image,
      scored_boxes_by_class_index = scored_boxes_by_class_index,
      class_index_to_name = coco.Dataset.class_index_to_name
    )
  ...
  def predict_all(self, model, split):
    options = self.options
    ...
    dataset = coco.Dataset(dir = options.dataset_dir, split = split, augment = False, shuffle = False)
  ...
  def __init__ (self, o, v = True):
    if type(o) is dict:
      options = DefaultMunch.fromDict(o)
    else:
      options = o
    ...
    model = faster_rcnn.FasterRCNNModel(
      ...
      num_classes            = coco.Dataset.num_classes,
    ...
    model.compile(optimizer = self.create_optimizer()) # losses not needed here because they were baked in at model construction
    ...
    if options.train:
      ...
      self.evaluate(model = model, plot = options.plot, print_average_precisions = True)
    elif options.predict:
      self.predict_one(model = model, url = options.predict, show_image = True, output_path = None)
    elif options.predict_to_file:
      self.predict_one(model = model, url = options.predict_to_file, show_image = False, output_path = "predictions.png")
    elif options.predict_all:
      self.predict_all(model = model, split = options.predict_all)
